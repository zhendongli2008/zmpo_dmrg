...
# To be changed.
mol.tmpdir = "/scratch/global/zhendong/femo_bs2_0/"
mol.build()
...
sc2.MaxMs  = [1] + [50]*np + [100]*(2*np) + [100]*(2*np) 
ns = len(sc2.MaxMs)
sc2.Sweeps = range(ns)
sc2.Tols   = [1.e-2] + [1.e-3]*np + [1.e-4]*(2*np) + [1.e-5]*(2*np) 
sc2.Noises = [1.e-4] + [1.e-4]*np + [0.0]*(4*np)
sc2.coff = 0 
